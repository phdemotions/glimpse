...
| 36 | **Excel parsing extracted to CP-3.5 mini-CP, not bundled with CP-3** | CP-1 risk note proposed CP-2; CP-2 deferred to CP-3; CP-3 doc-review (2026-04-30) flagged that Excel + 8 templates + export is too much for one CP. Excel becomes its own focused CP-3.5. Lands between CP-3 merge and CP-4 start. **Implementation note (CP-3.5 ship):** went with SheetJS + CSV-string bridge into existing DuckDB `read_csv_auto`, not the DuckDB-WASM `excel` extension. Reason: DuckDB excel extension loads from a CDN at runtime, which conflicts with Decision #3 (no network during processing) and Decision #28 (no third-party telemetry). SheetJS bundles at install time, lazy-loads in-browser, no network |
...
| 38 | **User-testing gap before CP-3 ship: accepted trade-off** | CP-3 doc-review (Adversarial) flagged that 8 templates designed by one author for an unobserved audience is "thesis-defended-without-data" — and Decision #28 (no analytics) makes post-ship blind too. Accepted: ship CP-3 on planner intuition + the 5-CSV detection corpus; first 3 think-aloud sessions go on the CP-3.5 / post-v1 docket. Reasoning: shipping the wedge against ChatGPT velocity-wise outranks pre-merge user testing for a free, no-account tool. Revisit if the trade-off bites in user feedback. **Bit in feedback at CP-3.5 ship:** infographic-mode templates render visibly below the Dual Standard on bundled samples — surfaced visceral "click-away" reaction. Triggering a CP-3.6 award-winning UX audit before CP-4 begins |
| 39 | **Excel parser: SheetJS Community 0.20.3 from SheetJS CDN tarball** | npm `xlsx` was abandoned when SheetJS pulled npm publishing in 2023 over a registry-policy dispute; the registry copy is stuck at 0.18.5 with known prototype-pollution. Pinning to the SheetJS CDN tarball gets the maintained version while bundling at install time (no runtime CDN, preserves Decision #3). Lazy-loaded into a 493KB chunk so CSV/JSON users pay zero |
| 40 | **Vercel preview is a parallel staging environment, not the prod target** | GitHub Pages remains the v1 prod target (Decision #2). Vercel exists for live testing during dev — `glimpse-orpin.vercel.app`. `vite.config.ts` `base` is env-conditional (`/` on Vercel, `/glimpse/` for GH Pages, `GLIMPSE_BASE` overrides). Two deploys, one source — no drift. No analytics on the Vercel deploy either (Decision #28 still applies) |
| 41 | **CP-3.6 — Infographic UX audit before CP-4** | Visceral "click-away" feedback from CP-3.5 ship indicates Infographic templates render below the Dual Standard on real-shaped data (not just synthetic samples). Treat this as a discrete CP, not a polish pass folded into CP-5. Scope: walk all 8 templates against representative samples, capture every Apple-Design-Award-grade and Enterprise-SaaS-grade gap (typography, spacing, density, hierarchy, type rendering, label collision, color, dark-on-dark contrast, axis treatment, default mark sizes), then ship targeted Vega-Lite spec + component fixes. Lands before CP-4 because CP-4 caches infographic state — caching ugly is cheaper than caching beautiful only after un-caching ugly |
...
| CP-3 | **Infographic mode** — 8 templates as parametrized Vega-Lite specs (Single Stat via `mark: 'text'`), brand fills, social-share export (SVG/PNG/spec JSON with inlined font subsets), mode-default auto-switch when high-scoring template fits, reuses CP-2 view-source | [`2026-04-30-003-feat-infographic-mode-cp-3-plan.md`](2026-04-30-003-feat-infographic-mode-cp-3-plan.md) | Complete (#3 merged) |
| CP-3.5 | **Excel parsing** *(focused mini-CP)* — SheetJS lazy-load → CSV bridge → existing DuckDB read path; multi-sheet switcher in SchemaView | [`2026-05-06-001-feat-excel-support-cp-3-5-plan.md`](2026-05-06-001-feat-excel-support-cp-3-5-plan.md) | Complete ([PR #4](https://github.com/phdemotions/glimpse/pull/4)) |
| CP-3.6 | **Infographic UX audit** *(award-winning design pass)* — Apple-Design-Award + Enterprise-SaaS bar against all 8 templates on representative samples; ship targeted spec + component fixes (see Decision #41) | TBD | Active — pre-CP-4 blocker |
